...
# >>> REMINDER: set TESTING_MODE back to false (or remove it) once trading
# is confirmed fully live again. Nathan asked to be reminded of this. <<<
# ─────────────────────────────────────────────
TESTING_MODE            = os.environ.get("TESTING_MODE", "false").lower() == "true"
if TESTING_MODE:
    print("[TESTING_MODE] ⚠️ Daily trade cap, loss-streak block, and $ loss block are BYPASSED. Set TESTING_MODE=false when ready to go fully live.")
...
class ClientConfig:
    def __init__(self, cid: str, private_key: str, address: str, leverage: int,
                 size_pct: float, tg_token: str, tg_chat_id: str, dashboard_token: str = ""):
...
def sync_clients_from_crm():
    ...
    for entry in payload.get("clients", []):
        ...
        if cid in existing_by_id:
            ...
            c = ClientConfig(cid, private_key, address, lev, pct, tg_token, tg_chat, dashboard_token)
...
def _execute_trade_inner(direction: str, size_pct: float = None, size_usdt: float = None,
    ...
    if not TESTING_MODE and now_uk_full.weekday() == 4 and now_uk >= dt_time(19, 0):
        send_telegram(
    ...
    if not TESTING_MODE and now_uk >= NIGHT_BLOCK_START:
        send_telegram(
    ...
    if not TESTING_MODE and time.time() < block_until:
    ...
    if not TESTING_MODE and (daily_losses_block or real_losses_today >= DAILY_LOSS_STREAK_LIMIT):
        send_telegram(
    ...
    if not TESTING_MODE and stopped_today:
        send_telegram(
...
def execute_signal_trade(direction: str, label: str):
    ...
    # ── Time blocks ───────────────────────────────────────────────────
    if not TESTING_MODE and now_uk_full.weekday() == 4 and uk_now >= dt_time(19, 0):
        send_telegram(
