...
@webhook_app.route("/api/admin/clients", methods=["GET"])
def api_admin_clients():
    """Management-console feed for Nathan's own dashboard: a snapshot of
    every other client's balance/position, so his view can show 'my
    trading' + 'all client accounts' in one place.

    Gated to the MASTER_CLIENT_ID's own dashboard_token specifically —
    not just any valid client token — so no client can ever pull this
    and see everyone else's numbers, only Nathan viewing his own link.
    """
    token = (flask_request.args.get("t") or flask_request.headers.get("X-Dashboard-Token", "")).strip()
    if not token:
        return jsonify({"error": "Missing access link token"}), 401
    ...
    for c in CLIENTS:
        if c.id == MASTER_CLIENT_ID:
            continue  # that's Nathan's own Binance account, shown separately

        link = f"{flask_request.host_url.rstrip('/')}/?t={c.dashboard_token}" if c.dashboard_token else None

        if not c.is_configured():
            rows.append({"client_id": c.id, "status": "not_linked", "dashboard_url": link})
            continue
        ...
        try:
            ...
            rows.append({
                ...
                "today": {"trades": c.trades_today, "wins": c.wins_today},
                "dashboard_url": link,
            })
        except Exception as e:
            print(f"[Admin Clients] Error fetching client {c.id}: {e}")
            rows.append({"client_id": c.id, "status": "error", "dashboard_url": link})

    return jsonify({"clients": rows}), 200
