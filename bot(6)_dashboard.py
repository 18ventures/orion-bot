...
def api_client_dashboard():
    ...
    Pulls the requesting client's OWN Hyperliquid account state (balance,
    position, today's mirrored trade stats) — not the master Binance
    account. Uses the same client_get_balance()/hl_get_info() calls the
    /balance and /status Telegram commands already rely on, so the
    dashboard and Telegram numbers stay consistent for a given client.
    """
    if READ_API_SECRET:
