...
# ============================================================
# ORION CLIENT DASHBOARD — READ ONLY
# Does not place, cancel, modify, or close orders.
# ============================================================
@webhook_app.route("/api/client/dashboard", methods=["GET", "OPTIONS"])
def api_client_dashboard():
    if flask_request.method == "OPTIONS":
        return jsonify({"status": "ok"}), 200
    if not READ_API_SECRET:
        return jsonify({"error": "Dashboard authentication is not configured"}), 503
    provided = flask_request.headers.get("X-Secret", "")
    if not provided or not hmac.compare_digest(provided, READ_API_SECRET):
        return jsonify({"error": "Unauthorized"}), 401

    client_id = flask_request.args.get("client_id", "1").strip()
    client = next((c for c in CLIENTS if c.id == client_id), None)
    if client is None:
        return jsonify({"error": "Unknown client"}), 404
    if not client.is_configured():
        return jsonify({"error": "Client account is not configured"}), 503

    try:
        info = hl_get_info()
        state = info.user_state(client.address)
        ...
        for item in state.get("assetPositions", []):
            ...
            if pos.get("coin") == HL_COIN and float(pos.get("szi", 0)) != 0:
                ...
                position = {
                    ...
                    "unrealized_pnl_pct": (unreal / notional * 100) if notional else 0,
                }
                break

        trades_today = int(getattr(client, "trades_today", 0))
        wins_today = int(getattr(client, "wins_today", 0))
        losses_today = int(getattr(client, "losses_today", 0))
        ...
        print(f"[Dashboard API] Error for client {client_id}: {e}")
