...
def adjust_sl(new_sl_price: float) -> None:
    ...
    try:
        ...
        try:
            ...
            # Don't just assume a previous stop is still protecting the
            # position — that was only ever true when this call was
            # MOVING an existing SL. If this was the first SL attempt on
            # this position, there was never a previous stop to fall back
            # on, and the old hardcoded message here was flatly false.
            # Check what's actually live on the exchange before saying
            # anything reassuring.
            try:
                existing_stops = [o for o in get_open_algo_orders() if o.get("type") == "STOP_MARKET"]
            except Exception:
                existing_stops = None  # couldn't verify — don't assume either way
