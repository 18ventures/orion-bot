...
def api_client_dashboard():
    ...
    client_id "1" is reserved for Nathan's own Binance master account
    (the bot's own balance/position/stats) — not a Hyperliquid mirror.
    Every other client_id is looked up in CLIENTS and pulls that
    client's OWN Hyperliquid account state, using the same
    client_get_balance()/hl_get_info() calls the /balance and /status
    Telegram commands already rely on, so the dashboard and Telegram
    numbers stay consistent for a given client.
    """
    if READ_API_SECRET:
    ...
    client_id = (flask_request.args.get("client_id") or "1").strip()
